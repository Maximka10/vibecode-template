                  ...
                  {activeOrders.map((order) => {
                    ...
                    return (
                      <Card key={order.id} variant="solid" padding="md" hover>
                        ...
                        {order.telegram_client_id ? (
                          <div className="mt-3 flex items-center gap-2 rounded-xl border border-green-500/20 bg-green-500/8 px-3 py-2.5">
                            <span className="text-sm">💬</span>
                            <p className="text-xs text-green-300/90">Telegram подключён — статусы заказа приходят вам в чат</p>
                          </div>
                        ) : botUsername ? (
                          <a
                            href={`https://t.me/${botUsername}?start=${order.id}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="mt-3 flex items-center justify-center gap-2 rounded-xl border border-[#229ED9]/40 bg-[#229ED9]/15 px-3 py-2.5 text-xs font-semibold text-[#5dc1f0] transition hover:bg-[#229ED9]/25"
                          >
                            <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor"><path d="M9.78 18.65l.28-4.23 7.68-6.92c.34-.31-.07-.46-.52-.19L7.74 13.3 3.64 12c-.88-.25-.89-.86.2-1.3l15.97-6.16c.73-.33 1.43.18 1.15 1.3l-2.72 12.81c-.19.91-.74 1.13-1.5.71l-4.07-3.01-1.97 1.91c-.22.21-.4.39-.81.39z"/></svg>
